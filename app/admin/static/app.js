...
// The Analytics export is a real downloadable PDF, generated byte-by-byte in
// the browser (no libraries, no print dialog, works offline). It draws the
// analytics tab — KPI cards, a donut + area trend, and the demand/question
// rankings — using PDF vector primitives (rounded rects, arcs, polygons) and
// the built-in Helvetica fonts, so the whole file stays ASCII and downloads
// as a Blob.

// Helvetica + Helvetica-Bold advance widths (per 1000 em) for ASCII 32..126,
// so labels/numbers can be measured for right-alignment and centring.
const _HELV_W = [278,278,355,556,556,889,667,191,333,333,389,584,278,333,278,278,556,556,556,556,556,556,556,556,556,556,278,278,584,584,584,556,1015,667,667,722,722,667,611,778,722,278,500,667,556,833,722,778,667,778,722,667,611,722,667,944,667,667,611,278,278,278,469,556,333,556,556,500,556,556,278,556,556,222,222,500,222,833,556,556,556,556,333,500,278,556,500,722,500,500,500,334,260,334,584];
const _HELV_BW = [278,333,474,556,556,889,722,238,333,333,389,584,278,333,278,278,556,556,556,556,556,556,556,556,556,556,333,333,584,584,584,611,975,722,722,722,722,667,611,778,722,278,556,722,611,833,722,778,667,778,722,667,611,722,667,944,667,667,611,333,278,333,584,556,333,556,611,556,611,556,333,611,611,278,278,556,278,889,611,611,611,611,389,556,333,611,556,778,556,556,500,389,280,389,584];

function _pdfTextWidth(str, size, bold) {
  const table = bold ? _HELV_BW : _HELV_W;
  let w = 0;
...
// Map the few non-ASCII glyphs we use to ASCII, then drop anything still
// outside the printable range so byte offsets equal string indices.
...
function _pdfStr(str) {
  return _pdfAscii(str).replace(/\\/g, "\\\\").replace(/\(/g, "\\(").replace(/\)/g, "\\)");
}
...
function _createPdf(title) {
  ...
  function pushPage() { if (s) pages.push(s); s = ""; y = M; }
  function ensure(h) { if (y + h > PH - M) pushPage(); }

  function rect(x, top, w, h, hex, mode) {
    const [r, g, b] = _hexRgb(hex);
    const op = mode === "stroke" ? "S" : "f";
    const color = mode === "stroke" ? `${fmt(r)} ${fmt(g)} ${fmt(b)} RG` : `${fmt(r)} ${fmt(g)} ${fmt(b)} rg`;
    s += `${color}\n${fmt(x)} ${fmt(ty(top + h))} ${fmt(w)} ${fmt(h)} re ${op}\n`;
  }
  function line(x1, t1, x2, t2, hex, width) {
    const [r, g, b] = _hexRgb(hex);
    s += `${fmt(width || 1)} w ${fmt(r)} ${fmt(g)} ${fmt(b)} RG\n${fmt(x1)} ${fmt(ty(t1))} m ${fmt(x2)} ${fmt(ty(t2))} l S\n`;
  }
  function text(x, top, str, size, bold, hex, align) {
    const w = _pdfTextWidth(_pdfAscii(str), size, bold);
    let tx = x;
    if (align === "right") tx = x - w;
    else if (align === "center") tx = x - w / 2;
    const [r, g, b] = _hexRgb(hex || "#1f2d44");
    const baseline = top + size * 0.8;
    s += `BT /${bold ? "F2" : "F1"} ${fmt(size)} Tf ${fmt(r)} ${fmt(g)} ${fmt(b)} rg ${fmt(tx)} ${fmt(ty(baseline))} Td (${_pdfStr(str)}) Tj ET\n`;
  }
  ...
  function fillPoly(pts, hex) {
    const [r, g, b] = _hexRgb(hex);
    s += `${fmt(r)} ${fmt(g)} ${fmt(b)} rg\n`;
    pts.forEach((p, i) => { s += `${fmt(p[0])} ${fmt(ty(p[1]))} ${i ? "l" : "m"}\n`; });
    s += "h f\n";
  }
  function strokePoly(pts, hex, width, closed) {
    const [r, g, b] = _hexRgb(hex);
    s += `1 J 1 j ${fmt(width)} w ${fmt(r)} ${fmt(g)} ${fmt(b)} RG\n`;
    pts.forEach((p, i) => { s += `${fmt(p[0])} ${fmt(ty(p[1]))} ${i ? "l" : "m"}\n`; });
    s += closed ? "h S\n" : "S\n";
  }
  ...
  return {
    ...
    finish() {
      ...
      const kids = [];
      let n = 5;
      footer.forEach((stream) => {
        const cNum = n++, pNum = n++;
        bodies[cNum] = `<< /Length ${stream.length} >>\nstream\n${stream}endstream`;
        bodies[pNum] = `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${fmt(PW)} ${fmt(PH)}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents ${cNum} 0 R >>`;
        kids.push(`${pNum} 0 R`);
      });
      bodies[2] = `<< /Type /Pages /Kids [${kids.join(" ")}] /Count ${p} >>`;
      const maxN = n - 1;
      ...
      pdf += `xref\n0 ${maxN + 1}\n0000000000 65535 f \n`;
      for (let i = 1; i <= maxN; i++) pdf += `${String(offsets[i]).padStart(10, "0")} 00000 n \n`;
...
function _analyticsPdf() {
  ...
  // Header.
  doc.text(M, doc.y, "Analytics Report", 20, true, "#0d213c");
  doc.text(right, doc.y + 2, "KOFON", 14, true, "#132178", "right");
  doc.y += 24;
  doc.text(M, doc.y, "AI Agent impact across service efficiency, product demand, lead creation, and satisfaction.", 9, false, "#5a6b85");
  doc.text(right, doc.y, "Generated " + generated, 8, false, "#8aa0bd", "right");
  doc.y += 14;
  ...
  function lineChart(values, x0, w, top, h) {
    ...
    const labs = ["30d ago", "20d ago", "10d ago", "Today"];
    labs.forEach((lab, i) => {
  ...
  doc.y = bt + 150;
  const stats = [[tMax + "%", "Peak"], [tMin + "%", "Low"], [tAvg + "%", "Average"], [(tChange >= 0 ? "+" : "") + tChange + " pts", "30-day change"]];
  ...
  heading("Product consultation ranking");
  doc.text(M, doc.y, pcTotal + " consultations total", 9, false, "#8aa0bd");
  doc.y += 14;
  barChart(pc, pcTotal, "#1d8cff");
  table(pc, pcTotal, ["Product line", "Consultations", "Share"]);

  heading("High frequency questions");
  doc.text(M, doc.y, qfTotal + " questions sampled", 9, false, "#8aa0bd");
  doc.y += 14;
  barChart(qf, qfTotal, "#32c5ff");
  table(qf, qfTotal, ["Question topic", "Count", "Share"]);
...
function exportAnalyticsReport() {
  ...
  // String is pure ASCII, so byte length == char length and the xref offsets
  // line up. The Uint8Array keeps it that way through the Blob.
