...
// Translate a data value to the active UI language for exports (CSV + PDF) so
// downloads read the same as the on-screen Chinese tables. Reuses the on-screen
// translation pass, so free-form text with no mapping (company names,
// summaries) is left as-is. Always returns a string.
function localize(value) {
  if (value == null) return "";
  return state.lang === "zh" ? translateLooseText(String(value)) : String(value);
}
...
// The Analytics export is a real downloadable PDF, generated in the browser (no
// libraries, no print dialog, works offline). Each page is drawn onto an
// offscreen canvas using the browser's own fonts — so reports render any
// script, including Chinese, with no embedded font — and is then rasterised
// into a JPEG image placed across the PDF page. The drawing API (rect / line /
// text / poly / donut) mirrors the report generators' expectations.

// Font stack: Latin UI faces first, then system CJK fonts across Win/mac/Linux.
const PDF_FONT = '"Helvetica Neue", "Segoe UI", Arial, "Microsoft YaHei", "PingFang SC", "Hiragino Sans GB", "Noto Sans CJK SC", "Noto Sans SC", sans-serif';

function _pdfFontSpec(size, bold) { return `${bold ? "700" : "400"} ${size}px ${PDF_FONT}`; }

// Width of a string as it will actually be drawn (after localisation), so
// right/centre alignment and truncation match the rendered glyphs.
...
function _pdfTextWidth(str, size, bold) {
  ...
  _pdfMeasureCtx.font = _pdfFontSpec(size, bold);
  return _pdfMeasureCtx.measureText(localize(str)).width;
}

// atob() yields a latin1 string (one char per byte); embedding it directly in
// the PDF string keeps `charCodeAt(i) & 0xff` byte-accurate on download.
function _binaryFromDataURL(dataUrl) {
  return atob(dataUrl.slice(dataUrl.indexOf(",") + 1));
}

// Top-origin page builder backed by one canvas per page. Strings drawn via
// text() are localised to the active UI language automatically.
function _createPdf(title) {
  const PW = 595.28, PH = 841.89, M = 42;
  const contentW = PW - 2 * M;
  const SCALE = 3; // ~216 DPI raster — crisp text at a reasonable file size
  ...
  // The current page is already retained in `ctxs`; pushPage just opens a new one.
  function pushPage() { newPage(); }
  function ensure(h) { if (y + h > PH - M) pushPage(); }

  function rect(x, top, w, h, hex, mode) {
    if (mode === "stroke") { ctx.strokeStyle = hex; ctx.lineWidth = 1; ctx.strokeRect(x, top, w, h); }
    else { ctx.fillStyle = hex; ctx.fillRect(x, top, w, h); }
  }
  function line(x1, t1, x2, t2, hex, width) {
    ctx.strokeStyle = hex; ctx.lineWidth = width || 1;
    ctx.beginPath(); ctx.moveTo(x1, t1); ctx.lineTo(x2, t2); ctx.stroke();
  }
  function text(x, top, str, size, bold, hex, align) {
    ctx.font = _pdfFontSpec(size, bold);
    ctx.fillStyle = hex || "#1f2d44";
    ctx.textAlign = align === "right" ? "right" : align === "center" ? "center" : "left";
    ctx.textBaseline = "alphabetic";
    ctx.fillText(localize(str), x, top + size * 0.8);
  }

  // Sample a circular arc (degrees, y-down space) into points.
  function arcPts(cx, cyTop, R, a0, a1, steps) {
    const out = [];
    for (let i = 0; i <= steps; i++) {
      const a = ((a0 + (a1 - a0) * (i / steps)) * Math.PI) / 180;
      out.push([cx + R * Math.cos(a), cyTop + R * Math.sin(a)]);
    }
    return out;
  }
  function roundRectPts(x, top, w, h, r) {
    r = Math.min(r, w / 2, h / 2);
    return [].concat(
      arcPts(x + r, top + r, r, 180, 270, 4),
      arcPts(x + w - r, top + r, r, 270, 360, 4),
      arcPts(x + w - r, top + h - r, r, 0, 90, 4),
      arcPts(x + r, top + h - r, r, 90, 180, 4)
    );
  }
  ...
  function fillPoly(pts, hex) { tracePoly(pts); ctx.closePath(); ctx.fillStyle = hex; ctx.fill(); }
  function strokePoly(pts, hex, width, closed) {
    tracePoly(pts); if (closed) ctx.closePath();
    ctx.strokeStyle = hex; ctx.lineWidth = width; ctx.stroke();
  }
  function roundRectFill(x, top, w, h, r, hex) { fillPoly(roundRectPts(x, top, w, h, r), hex); }
  function roundRectStroke(x, top, w, h, r, hex, width) { strokePoly(roundRectPts(x, top, w, h, r), hex, width || 1, true); }
  function donut(cx, cyTop, R, thick, pct, fg, bg) {
    strokePoly(arcPts(cx, cyTop, R, 0, 360, 60), bg, thick, true);
    const sweep = (Math.max(0, Math.min(100, pct)) / 100) * 360;
    if (sweep > 0) strokePoly(arcPts(cx, cyTop, R, -90, -90 + sweep, Math.max(2, Math.round(sweep / 6))), fg, thick, false);
  }
  ...
  return {
    ...
    get y() { return y; },
    set y(v) { y = v; },
    ensure, rect, line, text, pushPage,
    fillPoly, strokePoly, roundRectFill, roundRectStroke, donut,
    finish() {
      const total = ctxs.length;
      const footerLabel = msg("KOFON AI Agent Console", "KOFON AI 智能体控制台") + " - " + localize(docTitle);
      // Footer (line + caption + page number) drawn now that the total is known.
      ctxs.forEach((c, i) => {
        c.strokeStyle = "#8aa0bd"; c.lineWidth = 0.5;
        c.beginPath(); c.moveTo(M, PH - M + 14); c.lineTo(PW - M, PH - M + 14); c.stroke();
        c.font = _pdfFontSpec(8, false); c.fillStyle = "#8aa0bd"; c.textBaseline = "alphabetic";
        c.textAlign = "left";
        c.fillText(footerLabel, M, PH - M + 24);
        c.textAlign = "right";
        c.fillText(`${msg("Page", "第")} ${i + 1} / ${total}${msg("", " 页")}`, PW - M, PH - M + 24);
      });

      // Assemble: catalog, pages tree, then per page an image XObject + a
      // content stream that stretches the JPEG across the whole MediaBox.
      const objs = []; // 1-indexed via push; objs[i-1] is the body of object i
      const put = (body) => { objs.push(body); return objs.length; };
      const catalog = put(null), pagesObj = put(null);
      const kids = [];
      ctxs.forEach((c) => {
        const jpeg = _binaryFromDataURL(c.canvas.toDataURL("image/jpeg", 0.85));
        const imgNum = put(
          `<< /Type /XObject /Subtype /Image /Width ${c.canvas.width} /Height ${c.canvas.height} ` +
          `/ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpeg.length} >>\n` +
          `stream\n${jpeg}\nendstream`
        );
        const stream = `q\n${fmt(PW)} 0 0 ${fmt(PH)} 0 0 cm\n/Im0 Do\nQ\n`;
        const contentNum = put(`<< /Length ${stream.length} >>\nstream\n${stream}endstream`);
        const pageNum = put(
          `<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 ${fmt(PW)} ${fmt(PH)}] ` +
          `/Resources << /XObject << /Im0 ${imgNum} 0 R >> >> /Contents ${contentNum} 0 R >>`
        );
        kids.push(`${pageNum} 0 R`);
      });
      objs[catalog - 1] = `<< /Type /Catalog /Pages ${pagesObj} 0 R >>`;
      objs[pagesObj - 1] = `<< /Type /Pages /Kids [${kids.join(" ")}] /Count ${total} >>`;
...
function _analyticsPdf() {
  ...
  const pcTotal = pc.reduce((a, i) => a + i.value, 0) || 1;
  const qfTotal = qf.reduce((a, i) => a + i.value, 0) || 1;
  const tMin = Math.min(...responseTrend), tMax = Math.max(...responseTrend);
  const tAvg = Math.round(responseTrend.reduce((a, b) => a + b, 0) / responseTrend.length);
  const tChange = responseTrend[responseTrend.length - 1] - responseTrend[0];
  const generated = new Date().toLocaleString(state.lang === "zh" ? "zh-CN" : undefined);

  // Header.
  doc.text(M, doc.y, msg("Analytics Report", "分析报告"), 20, true, "#0d213c");
  doc.text(right, doc.y + 2, "KOFON", 14, true, "#132178", "right");
  doc.y += 24;
  doc.text(M, doc.y, msg("AI Agent impact across service efficiency, product demand, lead creation, and satisfaction.", "AI 智能体在服务效率、产品需求、线索创建与满意度方面的影响。"), 9, false, "#5a6b85");
  doc.text(right, doc.y, msg("Generated ", "生成时间 ") + generated, 8, false, "#8aa0bd", "right");
  doc.y += 14;
  ...
  function kpiGrid(cards) {
    ...
    cards.forEach((c, i) => {
      ...
      const sub = [c.delta, localize(c.sub)].filter(Boolean).join(" - ");
  ...
  function lineChart(values, x0, w, top, h) {
    ...
    const labs = [msg("30d ago", "30天前"), msg("20d ago", "20天前"), msg("10d ago", "10天前"), msg("Today", "今天")];
    labs.forEach((lab, i) => {
      const x = x0 + padL + (i / (labs.length - 1)) * plotW;
      doc.text(x, top + plotH + 4, lab, 8, false, "#8aa0bd", i === 0 ? "left" : i === labs.length - 1 ? "right" : "center");
    });
    const pts = values.map((v, i) => [xAt(i), yAt(v)]);
    const area = pts.concat([[xAt(values.length - 1), top + plotH], [xAt(0), top + plotH]]);
    doc.fillPoly(area, "#e8f3ff");
    doc.strokePoly(pts, "#1d8cff", 2, false);
    values.forEach((v, i) => doc.roundRectFill(xAt(i) - 2, yAt(v) - 2, 4, 4, 2, "#1d8cff"));
  }

  function barChart(items, total, hex) {
    const rowH = 26, labelW = 150, valW = 64;
    const barMax = CW - labelW - valW;
    const max = Math.max(...items.map((i) => i.value), 1);
    doc.ensure(items.length * rowH + 4);
    items.forEach((it) => {
      const top = doc.y, cy = top + rowH / 2;
      const w = Math.max(4, (it.value / max) * barMax);
      doc.text(M + labelW - 8, cy - 4, it.label, 10, false, "#33425c", "right");
      doc.roundRectFill(M + labelW, cy - 6, barMax, 12, 6, "#eef2f8");
      doc.roundRectFill(M + labelW, cy - 6, w, 12, 6, hex);
      const pct = ((it.value / total) * 100).toFixed(1) + "%";
      doc.text(M + labelW + w + 8, cy - 4, `${it.value} (${pct})`, 9, true, "#1f2d44");
      doc.y = top + rowH;
    });
    doc.y += 6;
  }

  function table(items, total, headers) {
    const rowH = 18, c2 = M + CW * 0.66, c3 = M + CW;
    doc.ensure(rowH * (items.length + 2));
    let top = doc.y;
    doc.roundRectFill(M, top, CW, rowH, 4, "#f7faff");
    doc.text(M + 8, top + 5, headers[0], 9, true, "#5a6b85");
    doc.text(c2, top + 5, headers[1], 9, true, "#5a6b85", "right");
    doc.text(c3 - 8, top + 5, headers[2], 9, true, "#5a6b85", "right");
    top += rowH;
    items.forEach((it) => {
      doc.text(M + 8, top + 4, it.label, 9, false, "#1f2d44");
      doc.text(c2, top + 4, String(it.value), 9, false, "#1f2d44", "right");
      doc.text(c3 - 8, top + 4, ((it.value / total) * 100).toFixed(1) + "%", 9, false, "#1f2d44", "right");
      doc.line(M, top + rowH, c3, top + rowH, "#eef2f8", 0.6);
      top += rowH;
    });
    doc.line(M, top, c3, top, "#e7edf5", 1.4);
    doc.text(M + 8, top + 4, msg("Total", "合计"), 9, true, "#0f2745");
    doc.text(c2, top + 4, String(total), 9, true, "#0f2745", "right");
    doc.text(c3 - 8, top + 4, "100%", 9, true, "#0f2745", "right");
    doc.y = top + rowH + 8;
  }

  heading(msg("Key results (last 30 days)", "近 30 天关键结果"));
  kpiGrid(analytics.valueCards);

  heading(msg("Operational overview", "运营概览"));
  kpiGrid(metrics);

  heading(msg("AI resolve rate", "AI 解决率"));
  const resolveCard = analytics.valueCards.find((c) => /resolution/i.test(c.label));
  const resolvePct = resolveCard ? parseFloat(resolveCard.value) : tMax;
  doc.ensure(176);
  const bt = doc.y;
  doc.donut(M + 64, bt + 62, 50, 15, resolvePct, "#1d8cff", "#eef2f8");
  doc.text(M + 64, bt + 54, resolvePct + "%", 18, true, "#0f2745", "center");
  doc.text(M + 64, bt + 130, msg("AI resolution rate", "AI 解决率"), 8, false, "#5a6b85", "center");
  lineChart(responseTrend, M + 150, CW - 150, bt + 4, 138);
  doc.y = bt + 150;
  const stats = [[tMax + "%", msg("Peak", "峰值")], [tMin + "%", msg("Low", "谷值")], [tAvg + "%", msg("Average", "平均")], [(tChange >= 0 ? "+" : "") + tChange + msg(" pts", " 点"), msg("30-day change", "30 天变化")]];
  const sw = CW / 4, stop = doc.y;
  stats.forEach((st, i) => {
    doc.text(M + i * sw, stop, st[0], 14, true, "#0f2745");
    doc.text(M + i * sw, stop + 16, st[1], 8, false, "#5a6b85");
  });
  doc.y = stop + 34;

  heading(msg("Product consultation ranking", "产品咨询排名"));
  doc.text(M, doc.y, pcTotal + msg(" consultations total", " 次咨询合计"), 9, false, "#8aa0bd");
  doc.y += 14;
  barChart(pc, pcTotal, "#1d8cff");
  table(pc, pcTotal, [msg("Product line", "产品线"), msg("Consultations", "咨询数"), msg("Share", "占比")]);

  heading(msg("High frequency questions", "高频问题"));
  doc.text(M, doc.y, qfTotal + msg(" questions sampled", " 个问题抽样"), 9, false, "#8aa0bd");
  doc.y += 14;
  barChart(qf, qfTotal, "#32c5ff");
  table(qf, qfTotal, [msg("Question topic", "问题主题"), msg("Count", "数量"), msg("Share", "占比")]);

  return doc.finish();
}

function exportAnalyticsReport() {
  const pdf = _analyticsPdf();
  // The PDF string holds only chars 0-255 (ASCII structure + latin1 JPEG bytes),
  // so byte length == char length and the xref offsets line up.
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `kofon-analytics-${new Date().toISOString().slice(0, 10)}.pdf`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 0);
...
function _dailyBriefPdf() {
  ...
  function lineChart(values, x0, w, top, h) {
    ...
    [msg("30d ago", "30天前"), msg("20d ago", "20天前"), msg("10d ago", "10天前"), msg("Today", "今天")].forEach((lab, i, arr) => {
  ...
  function fit(str, maxW) {
    // Localise first so truncation measures the glyphs that will actually print.
    str = localize(str);
  ...
  // Header.
  const today = new Date().toLocaleDateString(state.lang === "zh" ? "zh-CN" : undefined, { weekday: "long", year: "numeric", month: "long", day: "numeric" });
  doc.text(M, doc.y, msg("Daily Brief", "每日简报"), 20, true, "#0d213c");
  doc.text(right, doc.y + 2, "KOFON", 14, true, "#132178", "right");
  doc.y += 24;
  doc.text(M, doc.y, today, 9, false, "#5a6b85");
  doc.text(right, doc.y, msg("Generated ", "生成时间 ") + new Date().toLocaleString(state.lang === "zh" ? "zh-CN" : undefined), 8, false, "#8aa0bd", "right");
  doc.y += 14;
  doc.line(M, doc.y, right, doc.y, "#132178", 2);
  doc.y += 18;

  // Today at a glance.
  heading(msg("Today at a glance", "今日概览"));
  kpiGrid(metrics);

  // Conversations.
  heading(msg("Conversations", "对话"));
  ...
  note(conversations.length + msg(" conversations in view - status breakdown", " 条对话（视图内）— 状态分布"));
  bars(convItems, conversations.length, "#1d8cff");

  // AI quality.
  heading(msg("AI quality", "AI 质量"));
  const resolveCard = analytics.valueCards.find((c) => /resolution/i.test(c.label));
  ...
  lineChart(responseTrend, M + 150, CW - 150, bt + 4, 138);
  doc.y = bt + 150;
  note(answerReviews.length + msg(" answers flagged for review", " 条回答待审核"));

  // Product demand + top questions.
  heading(msg("Product demand", "产品需求"));
  ...
  // Sales pipeline.
  heading(msg("Sales pipeline", "销售管道"));
  kpiGrid([
    { label: msg("Open leads", "未关闭线索"), value: leads.length },
    { label: msg("A-level leads", "A 级线索"), value: cnt(leads, (l) => l.level === "A") },
    { label: msg("Customers", "客户"), value: customers.length },
    { label: msg("Unassigned", "未分配"), value: cnt(leads, (l) => /unassigned/i.test(l.owner)) },
  ]);
  table(
    [msg("Customer", "客户"), msg("Company", "公司"), msg("Product", "产品"), msg("Lvl", "等级"), msg("Owner", "负责人")],
    leads.map((l) => [l.customer, l.company, l.product, l.level, l.owner]),
    [0.22, 0.27, 0.26, 0.07, 0.18]
  );

  // Knowledge, content & catalog.
  heading(msg("Knowledge & content", "知识与内容"));
  kpiGrid([
    { label: msg("Documents", "文档"), value: `${cnt(documents, (d) => d.status === "Enabled")}/${documents.length}`, sub: msg("enabled", "已启用") },
    { label: msg("FAQs", "常见问题"), value: faqs.length, sub: msg("published", "已发布") },
    { label: msg("Products", "产品"), value: products.length, sub: msg("in catalog", "在目录中") },
    { label: msg("Reviews open", "待审核"), value: answerReviews.length, sub: msg("answer review", "回答审核") },
  ]);
  note(msg("Product catalog by status", "产品目录（按状态）"));
  const statusCounts = {};
  ...
  if (adminUsers.length) {
    ...
    note(msg("Administrator list not loaded — open User Permissions to include it.", "管理员列表未加载 — 打开「用户权限」以包含该部分。"));
  }

  // Agent configuration.
  heading(msg("Agent configuration", "智能体配置"));
  doc.text(M, doc.y, msg("Agent name: ", "智能体名称：") + agentSettings.agent_name, 10, false, "#1f2d44"); doc.y += 16;
  doc.text(M, doc.y, msg("Languages: ", "语言：") + (agentSettings.enabled_languages || []).join(", "), 10, false, "#1f2d44"); doc.y += 16;
  doc.text(M, doc.y, msg("Auto-create lead: ", "自动创建线索：") + (agentSettings.auto_create_lead ? msg("On", "开") : msg("Off", "关")) +
    msg("      Confidence threshold: ", "      置信度阈值：") + (agentSettings.require_confidence_threshold ? msg("On", "开") : msg("Off", "关")), 10, false, "#1f2d44");
  doc.y += 20;

  // Recent operations.
  heading(msg("Recent operations", "近期操作"));
  if (logs.length) {
    table(
      [msg("Time", "时间"), msg("Actor", "操作者"), msg("Action", "操作"), msg("Target", "对象")],
      logs.slice(0, 8).map((l) => [l.time, l.actor, l.action, l.target]),
      [0.24, 0.22, 0.3, 0.24]
    );
  } else {
    note(msg("No operation log entries available.", "暂无操作日志记录。"));
  }
...
// Horizontal bars scaled so the top item fills the track (its bar = 100%) and
// the rest are relative to it; each row also shows its share of the total. The
// `total` (for the share %) defaults to the sum of values when not supplied.
function barList(items, opts = {}) {
  const values = items.map((item) => (item.value != null ? item.value : item.rate));
  const max = Math.max(...values, 1);
  const total = opts.total != null ? opts.total : values.reduce((sum, v) => sum + v, 0) || 1;
  return `
    <div class="bar-list">
      ${items
        .map((item, i) => {
          ...
          const count = item.count != null ? item.count : value;
          return `
            <div class="bar-row">
              <div>
              ...
              <b>${escapeHtml(String(count))} <small class="bar-share">${share}%</small></b>
            </div>
          `;
        })
        .join("")}
...
function toggleableChart(id, items, opts = {}) {
  return chartView(id) === "pie" ? pieChart(items, opts) : barList(items, opts);
}
...
root.addEventListener("click", (event) => {
  ...
  if (action === "export-dashboard") {
    ...
    metrics.forEach((m) => rows.push([localize(m.label), localize(m.value), localize(m.delta || ""), localize(m.sub || "")]));
  ...
  if (action === "export-conversations") {
    ...
    conversations.forEach((c) =>
      rows.push([c.id, localize(c.user), localize(c.company), localize(c.product), localize(c.time), localize(c.status), localize(c.summary)])
    );
  ...
  if (action === "export-logs") {
    ...
    logs.forEach((l) =>
      rows.push([localize(l.time), localize(l.actor), localize(l.action), localize(l.target), localize(l.details || ""), localize(l.result)])
    );
