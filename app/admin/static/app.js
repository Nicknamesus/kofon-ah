...
// analytics tab — KPI cards, the resolve-rate trend, and the demand/question
// rankings — using only PDF text/line/rect primitives and the built-in
// Helvetica fonts, so the whole file stays ASCII and downloads as a Blob.
...
function _createPdf() {
  ...
  return {
    ...
    finish() {
      ...
      // then a (content, page) pair per page.
      const bodies = {};
...
function _analyticsPdf() {
  ...
  function kpiGrid(cards) {
    const cols = 4, gap = 12, cardW = (CW - gap * (cols - 1)) / cols, cardH = 60;
    cards.forEach((c, i) => {
      const col = i % cols;
      if (col === 0) { doc.ensure(cardH + 4); }
      const x = M + col * (cardW + gap);
      const top = doc.y;
      doc.rect(x, top, cardW, cardH, "#fbfdff", "fill");
      doc.rect(x, top, cardW, cardH, "#e7edf5", "stroke");
      doc.text(x + 10, top + 10, c.label, 8, false, "#5a6b85");
      doc.text(x + 10, top + 22, String(c.value), 16, true, "#0f2745");
      const sub = [c.delta, c.sub].filter(Boolean).join(" - ");
      if (sub) doc.text(x + 10, top + 45, sub, 8, false, "#8aa0bd");
      if (col === cols - 1 || i === cards.length - 1) doc.y = top + cardH + gap;
    });
  }

  function barChart(items, total, hex, unit) {
    const rowH = 26, labelW = 150, valW = 46;
    ...
    items.forEach((it) => {
      ...
      doc.rect(M + labelW, cy - 6, barMax, 12, "#eef2f8", "fill");
      doc.rect(M + labelW, cy - 6, w, 12, hex, "fill");
  ...
  function lineChart(values) {
    const h = 150, padL = 32, padB = 18;
    doc.ensure(h + 4);
    const top = doc.y;
    const plotW = CW - padL, plotH = h - padB;
    const max = Math.max(...values, 0) || 1;
    const xAt = (i) => M + padL + (i * plotW) / (values.length - 1);
    const yAt = (v) => top + plotH - (v / max) * plotH;
    [0, max / 2, max].forEach((t) => {
      const yy = top + plotH - (t / max) * plotH;
      doc.line(M + padL, yy, M + CW, yy, "#e7edf5", 0.6);
      doc.text(M + padL - 6, yy - 4, String(Math.round(t)), 8, false, "#8aa0bd", "right");
    });
    ["30d ago", "20d ago", "10d ago", "Today"].forEach((lab, i, arr) => {
      const x = M + padL + (i / (arr.length - 1)) * plotW;
      doc.text(x, top + plotH + 4, lab, 8, false, "#8aa0bd", i === 0 ? "left" : i === arr.length - 1 ? "right" : "center");
    });
    for (let i = 1; i < values.length; i++) {
      doc.line(xAt(i - 1), yAt(values[i - 1]), xAt(i), yAt(values[i]), "#1d8cff", 2);
    }
    values.forEach((v, i) => doc.rect(xAt(i) - 1.6, yAt(v) - 1.6, 3.2, 3.2, "#1d8cff", "fill"));
    doc.y = top + h + 4;
  }
  ...
  function table(items, total, headers) {
    ...
    let top = doc.y;
    doc.rect(M, top, CW, rowH, "#f7faff", "fill");
    doc.text(M + 6, top + 5, headers[0], 9, true, "#5a6b85");
    doc.text(c2, top + 5, headers[1], 9, true, "#5a6b85", "right");
    doc.text(c3 - 6, top + 5, headers[2], 9, true, "#5a6b85", "right");
    top += rowH;
    items.forEach((it) => {
      doc.text(M + 6, top + 4, it.label, 9, false, "#1f2d44");
      doc.text(c2, top + 4, String(it.value), 9, false, "#1f2d44", "right");
      doc.text(c3 - 6, top + 4, ((it.value / total) * 100).toFixed(1) + "%", 9, false, "#1f2d44", "right");
      doc.line(M, top + rowH, c3, top + rowH, "#eef2f8", 0.6);
      top += rowH;
    });
    doc.line(M, top, c3, top, "#e7edf5", 1.4);
    doc.text(M + 6, top + 4, "Total", 9, true, "#0f2745");
    doc.text(c2, top + 4, String(total), 9, true, "#0f2745", "right");
    doc.text(c3 - 6, top + 4, "100%", 9, true, "#0f2745", "right");
    doc.y = top + rowH + 8;
  }
  ...
  doc.text(M, doc.y, resolvePct + "%", 22, true, "#0f2745");
  doc.text(M + 70, doc.y + 6, "AI resolution rate, last 30 days", 9, false, "#5a6b85");
  doc.y += 26;
  doc.rect(M, doc.y, CW, 10, "#eef2f8", "fill");
  doc.rect(M, doc.y, CW * (resolvePct / 100), 10, "#1d8cff", "fill");
  doc.y += 22;
  lineChart(responseTrend);
  // Stat row.
...
function exportAnalyticsReport() {
  ...
  // line up. latin1 keeps it that way through the Blob.
